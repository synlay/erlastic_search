...
count(Params, Index, Type, Query, Opts) when is_binary(Query) ->
    search_helper(<<"_count">>, Params, Index, Type, Query, Opts);
count(Params, Index, Type, Query, Opts) ->
    count(Params, Index, Type, erls_json:encode(Query), Opts).

-spec count_query(#erls_params{}, list() | binary(), list() | binary(), binary(), list()) -> {ok, erlastic_success_result()} | {error, any()}.
count_query(Params, Index, Type, Query, Opts) ->
    search_query_helper(<<"_count">>, Params, Index, Type, Query, Opts).
...
search_query(Params, Index, Type, Query, Opts) ->
    search_query_helper(<<"_search">>, Params, Index, Type, Query, Opts).
...
search_helper(Endpoint, Params, Index, Type, Query, Opts) ->
    erls_resource:get(Params, filename:join([commas(Index), Type, Endpoint]), [], Opts, Query, Params#erls_params.http_client_options).

-spec search_query_helper(binary(), #erls_params{}, list() | binary(), list() | binary(), erlastic_json() | binary(), list()) -> {ok, erlastic_success_result()} | {error, any()}.
search_query_helper(Endpoint, Params, Index, Type, Query, Opts) ->
